...
var Pipeline = require('gltf-pipeline');
        ...
        .then(function() {
            ...
            if (!compress) {
                ...
                .then(function() {
                    ...
                    removeEmpty(gltf);
                    if (binary) {
                        clearEmbeddedBuffer(gltf);
                        return gltfToGlb(gltf, binaryBuffer);
                    }
                    return gltf;
                });
            }
            promises.push(compressGltf(gltf));
            return Promise.all(promises)
                .then(function(gltfArr) {
                    var gltf = gltfArr[0];
                    if (binary) {
                        binaryBuffer = getEmbeddedBuffer(gltf);
                        return gltfToGlb(gltf, binaryBuffer);
                    }
                    return gltf;
                }).catch(function(err) {
...
function clearEmbeddedBuffer(gltf) {
    var buffer = gltf.buffers[0];
    if (buffer.uri) {
        delete buffer.uri;
    }
...
function getEmbeddedBuffer(gltf) {
    var buffer = gltf.buffers[0];
    if (buffer.uri) {
        var prefix = 'data:application/octet-stream;base64,';
        // var regex = /^data:.+\/(.+);base64,(.*)$/;
        // var matches = buffer.uri.match(regex);
        // var data = matches[2];
        // var buffer = Buffer.from(data, 'base64');
        var data = buffer.uri.substring(prefix.length);
...
function compressGltf(gltf) {
    deleteExtras(gltf);
    removeEmpty(gltf);
    return Pipeline.processGltf(gltf,  {
        dracoOptions: {
            compressionLevel: 10
        }
