...
function addPrimitive(gltf, materials, bufferState, uint32Indices, mesh, primitive, index, options) {
    ...
    var faceName = mesh.name;
    if (index > 0) {
        faceName = mesh.name + '_' + index;
    }

    var accessorIndex;
    var attributes = {};
    if (hasPositions) {
        accessorIndex = addVertexAttribute(gltf, primitive.positions, 3, faceName + '_positions');
    ...
    if (hasNormals) {
        accessorIndex = addVertexAttribute(gltf, primitive.normals, 3, faceName + '_normals');
    ...
    if (hasUVs) {
        accessorIndex = addVertexAttribute(gltf, primitive.uvs, 2, faceName + '_texcoords');
    ...
    if(useBatchId) {
        var batchIdCount = primitive.positions.length / 3;
        accessorIndex = addBacthIdAttribute(gltf, bufferState.currentBatchId, batchIdCount, faceName + '_batchId');
        ...
        bufferState.batchTableJson.name.push(faceName);
        bufferState.currentBatchId ++;
    }

    var indexAccessorIndex = addIndexArray(gltf, primitive.indices, uint32Indices, faceName + '_indices');
...
function addMesh(gltf, materials, bufferState, uint32Indices, mesh, options) {
    ...
    for (var i = 0; i < primitivesLength; ++i) {
        ...
        gltfPrimitives.push(addPrimitive(
            gltf, materials, bufferState, uint32Indices, mesh, primitives[i], i, options) );
